...
test('calls within the window increment until the cap', () => {
  const d =
    rateLimitDecision(1500, {windowStart: 1000, count: 2}, 3, WINDOW_MS);
  assert.equal(d.allow, true);
...
test('a call at the cap is blocked', () => {
  const d =
    rateLimitDecision(1500, {windowStart: 1000, count: 3}, 3, WINDOW_MS);
  assert.equal(d.allow, false);
  assert.equal(d.nextState, undefined);
});

test('a new window opens once the old one elapses, even if capped', () => {
  const now = 1000 + WINDOW_MS;
  const d =
    rateLimitDecision(now, {windowStart: 1000, count: 99}, 3, WINDOW_MS);
  assert.equal(d.allow, true);
